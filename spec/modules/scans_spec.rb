...
describe Resource::Scans do
          ...
          scans: [
            {
              ...
              type: nil,
              read: true,
              last_modification_date: 1_430_934_526,
              creation_date: 1_430_933_086,
              status: 'imported',
  ...
  context '.list_scans' do
    it 'successful list scans' do
      allow_any_instance_of(NessusClient::Request).to receive(
        ...
        { path: '/scans', query: nil, headers: be_kind_of(Hash) }
      ).and_return(@list_of_scans)
  ...
  context '.scan_details' do
    it 'successful get scan detail' do
      ...
      allow_any_instance_of(Resource::Session).to receive(
        :set_session
      ).with('username', 'password').and_return(
        'mock_auth_cookie'
      )
      allow_any_instance_of(NessusClient).to receive(
        :new
