...
describe Resource::Exports do
    ...
    it 'successful export request' do
      ...
      allow_any_instance_of(NessusClient::Request).to receive(
      ...
      ).with(
        {
          ...
          payload: { format: 'nessus' }, headers: be_kind_of(Hash)
        }
      ).and_return(
        Oj.dump(
          {
      ...
      expect(
        export_request['export_uuid']
        ).to eq('73376c41-1508-46b7-8587-483d159cd956')
    end
  end
  context '.export_status' do
    it 'successful export status' do
      allow_any_instance_of(NessusClient::Request).to receive(
        ...
        {
          path: '/scans/scan_id/export/file_id/status',
      ...
      nessus_client = NessusClient.new(@payload)
      export_status = Oj.load(
        nessus_client.export_status('scan_id', 'file_id')
      )
  ...
  context '.export_download' do
    it 'successful export_download' do
      ...
      nessus_client = NessusClient.new(@payload)
      export = Oj.load(nessus_client.export_download(1, 123_456))
