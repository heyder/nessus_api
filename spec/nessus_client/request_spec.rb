...
describe NessusClient::Request do
  before(:context) do
    @nessus_request = NessusClient::Request.new(
  ...
  context 'initialize' do
    it 'when valid uri' do
      ...
      ).to be_instance_of(NessusClient::Request)
    end
    ...
    it 'when nil on into initialize, should raise TypeError exception' do
      expect {
        NessusClient::Request.new(nil)
      }.to raise_error(TypeError)
      expect {
        NessusClient::Request.new({})
      }.to raise_error(URI::InvalidURIError)
    end

    it 'when uri is nil, should raise URI::InvalidURIError exception' do
      expect {
        NessusClient::Request.new({ uri: nil })
      }.to raise_error(URI::InvalidURIError)
    end
  end
  ...
  context '.headers' do
    ...
    it 'still default' do
      ...
      expect(
        NessusClient::Request::DEFAULT_HEADERS
  ...
  context '.get' do
    it 'response has a body' do
      allow(NessusClient::Request).to receive(
        :get
      ).and_return('RESPONSE_BODY')
      expect(
        NessusClient::Request.get
      ).to eq('RESPONSE_BODY')
    end
